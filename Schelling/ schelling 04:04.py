...
#Idée de programme 2: représenter graphiquement la ville avec des carrés, et représenter dans un second temps son évolution
...
def trace_ville (ville,i) :  # i est le numéro de la figure
    ...
    fig = plt.figure(i)
    ax = plt.axes(xlim=(-0.5,np.shape(ville)[1]-0.5), ylim=(-0.5,np.shape(ville)[0]-0.5)) #crée les axes des abscisses et ordonnées avec des longueurs adaptées à la taille de ma ville
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    ax.set_aspect("equal")
    
    #On place ensuite les individus :
    cmap = ListedColormap([(1,1,1),(0,0,0),(1,1,0)]) #couleurs à choisir selon le code RVB
    img = plt.imshow(ville, cmap=cmap) 
    plt.draw()
# ==> Résultat: la fonction trace_ville trace ma ville avec sa frontière extérieure et utilise le code couleur suivant: blanc si l'emplacement est vide, rouge si on a un habitant black, et vert si c'est un blanc
        

def simulation (ville,i_max,nb_graph) :  #choisis de sorte que i_max divisible par (nb_graph-1)
    trace_ville(ville,0)
    sub_i = i_max // (nb_graph-1)   # Nombre d'itérations entre chaque graphique
    for i in range(1,nb_graph) :
        for j in range(sub_i) :
            satisfaction(ville,np.random.randint(1,np.shape(ville)[0]-1),np.random.randint(1,np.shape(ville)[1]-1),1/2)
        trace_ville(ville,i)
    

        
#Idée de programme 3 (qui reprend essentiellement le premier) : on crée une ville de taille nxp avec sa frontière extérieure, on prend un citoyen au hasard dedans, s'il est satisfait de son voisinage il reste, sinon il se déplace vers le premier emplacement vide qu'il trouve et qui a un voisinage qui lui est satisfaisant.  
